...
(() => {
  ...
  const duplicateTabsExist = () =>
    getGBrowser() && getGBrowser().getAllDuplicateTabsToClose().length > 0;
  ...
  const removeDuplicatesAndNotify = () => {
    const gBrowser = getGBrowser();
    if (!gBrowser) return;
  ...
  const attachClickListener = (menuItemId) => {
  ...
  const showClosedTabsNotification = (closedTabCount) => {
